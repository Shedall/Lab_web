...
class LoginForm(AuthenticationForm):
    username = UsernameField(label="имя пользователя", max_length=65, widget=forms.TextInput(
...
class RegistrationForm(UserProfileCreationForm):
    terms_of_service_accepted = forms.BooleanField(
        required=False,
        label="согласен с политикой конфединциальности" "",
        help_text="Определяет, может ли пользователь предоставлять продукты."
    )
